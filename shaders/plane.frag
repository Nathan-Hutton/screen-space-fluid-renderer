...
void main()
{
    ...
    // if (dzy == 0) { color = vec4(1, 1, 1, 1); return; }

    // vec3 normal = normalize(cross(vec3(1, 0, dzx), vec3(0, 1, dzy)));
    vec3 normal = normalize(cross(vec3(dx, 0, -dzx), vec3(0, dy, -dzy)));
    // vec3 normal = normalize(vec3(dzx, dzy, 1));

    // if (dx <= 0) { color = vec4(1, 1, 1, 1); return; }
    // dx = dx * 1000;
    // color = vec4(dx, dx, dx, 1);
